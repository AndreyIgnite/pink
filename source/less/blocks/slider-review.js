...
let sliderReviews = document.querySelectorAll(".slider-review__item");
let sliderDots = document.querySelectorAll(".toggles__dot");
...
function switchDot(index) {
  sliderDots.forEach((item) => item.classList.remove("toggles__dot--current"));
  sliderDots[index].classList.add("toggles__dot--current");
}
...
touchSurface.addEventListener(
  ...
  function (e) {
    ...
    if (
      dragndropSlideShit < -sliderWidth / 7 &&
      !(sliderCount == sliderReviews.length - 1)
    ) {
      sliderCount++;
      if (sliderCount >= sliderReviews.length) {
